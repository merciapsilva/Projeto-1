...
sp_input <- read.csv(file = "./data/ocorrencias/ocorrencias/sp_input_setupsdmdata.csv", sep = ",") # 
#Colocar underline entre genero e espécie
sp_input$species <-
...
lista_arquivos <- list.files("./data/raster/raster/variaveis_cortadas_pratica/", full.names = T, pattern = ".tif")

vars_stack <-stack(lista_arquivos)

#Verificar todas as variáveis ambientais
plot(vars_stack)

#Selecionar só uma variavel
plot(vars_stack[[12]])
